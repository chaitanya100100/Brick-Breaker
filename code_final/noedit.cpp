...
{
	if(selected == obj["funnel"])
	{
		double x, y;
		get_coords(&x, &y);
		float r = atan2(y - c->get_y(), x - c->get_x());
		c->set_funnel_rotation(float(r*180.0f/M_PI));
	}
	else if(selected == obj["canon"])
	{
		double x, y;
		get_coords(&x, &y);
		c->set_y(y);
	}
	/*else if(selected == obj["red_bucket"] || selected == obj["green_bucket"])
	{
		double x, y;
		get_coords(&x, &y);
		cout << selected << " " << x << endl;
		//d[selected]->set_x(x);
		cout << d[selected]->get_x() << endl;
		//d[selected]->set_x(float(x));
	}*/
}
...
//        exit(EXIT_FAILURE);
    ...
    window = glfwCreateWindow(width, height, GameHeading, NULL, NULL);
